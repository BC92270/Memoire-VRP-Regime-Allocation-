...
All sensitivity comparisons preserve the timing rules described in the methodology. Direct-variance parameter grids are aligned to a common sample within each sensitivity dimension. Consequently, a 12-month and a 60-month risk-estimation specification are compared only over dates available to both. Full parameter grids are reported in the robustness appendix; the main text focuses on the results required to assess the economic conclusion.
